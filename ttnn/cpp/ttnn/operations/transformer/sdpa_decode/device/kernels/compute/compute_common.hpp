...
void reduce_c(uint32_t out_cb, uint32_t prev_cb, uint32_t cols, bool do_eltwise_max = false) {
    ...
    for (uint32_t r = 0; r < rows; ++r) {
        acquire_dst();
        reduce_init<pool_type, reduce_dim, fp32_dest_acc_en>(in0_cb, scale_cb, out_cb);
        for (uint32_t c = 0; c < cols; ++c) {
            reduce_tile<pool_type, reduce_dim, fp32_dest_acc_en>(in0_cb, scale_cb, r * cols + c, 0, reduce_dst_idx);
        }
        reduce_uninit();
        if (do_eltwise_max) {
