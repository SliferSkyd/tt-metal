...
void kernel_main() {
    ...
    // TensorAccessor setup for source and destination tensors
    constexpr auto src_tensor_args = TensorAccessorArgs<4>();
    constexpr auto dst_tensor_args = TensorAccessorArgs<src_tensor_args.next_compile_time_args_offset()>();

    const auto s = TensorAccessor(src_tensor_args, src_addr, original_page_size_bytes);
    const auto d = TensorAccessor(dst_tensor_args, dst_addr, dest_page_size_bytes);
