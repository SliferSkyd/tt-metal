...
def test_concat(device):
    ...
    print("Concatenated Tensor Shape:", concatenated_tensor.shape)  # Concatenated Tensor Shape: Shape([1, 1, 64, 64])
...
def test_pad(device):
    ...
    print("Padded Tensor Shape:", padded_tensor.shape)  # Padded Tensor Shape: Shape([1, 1, 16, 16])
...
def test_permute(device):
    ...
    print("Permuted Tensor Shape:", permuted_tensor.shape)  # Permuted Tensor Shape: Shape([1, 1, 32, 64])
...
def test_reshape(device):
    ...
    print("Reshaped Tensor Shape:", reshaped_tensor.shape)  # Reshaped Tensor Shape: Shape([1, 1, 2, 2])
...
def test_repeat_interleave(device):
    ...
    print("Repeat Interleave Tensor Shape:", repeated_tensor.shape)  # Repeat Interleave Tensor Shape: Shape([20, 10])
...
def test_slice(device):
    ...
    print("Sliced Tensor Shape:", sliced_tensor.shape)  # Sliced Tensor Shape: Shape([1, 1, 32, 16])

    # Create a tensor to slice without step
    input_tensor = ttnn.rand((1, 1, 64, 32), dtype=ttnn.bfloat16, layout=ttnn.Layout.TILE, device=device)
    output = ttnn.slice(input_tensor, [0, 0, 0, 0], [1, 1, 32, 32])
    print("Sliced Tensor Shape:", output.shape)  # Sliced Tensor Shape: Shape([1, 1, 32, 32])
