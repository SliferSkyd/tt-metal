...
/*
# Steps to verify
export TT_CACHE=home/prybicki/.cache/tt-metal-cache

# Build example demonstrating the feature:
./build_metal.sh --build-programming-examples

# Run normally to generate the cache:
./build/programming_examples/metal_example_add_2_integers_in_compute

# find .elf files in TT_CACHE - should contain user's binaries
find $TT_CACHE -path "*/kernels/*.elf"

# Move cache 
export MY_HASH=d437822a96
mv $TT_CACHE/$MY_HASH ~/

TT_BINARY_PATH=~/$MY_HASH ./build/programming_examples/metal_example_add_2_integers_in_compute

# find any .elf files - should not contain user's binaries (which means they were loaded from $TT_BINARY_PATH)
find $TT_CACHE -name "*.elf"
*/
...
int main() {
    ...
    KernelHandle binary_reader_kernel_id = CompileOrLoadKernel(
        program, core, kernel_binary_prefix,
