...
#include "ckernel_sfpu_ln_rsqrt.h"
...
inline void llk_math_eltwise_unary_sfpu_ln_rsqrt(uint dst_index, int vector_mode = (int)VectorMode::RC) {
    _llk_math_eltwise_unary_sfpu_params_<APPROXIMATE>(
        ckernel::sfpu::calculate_ln_rsqrt<APPROXIMATE, 8, 2, is_fp32_dest_acc_en>, dst_index, vector_mode);
}

template <bool APPROXIMATE>
inline void llk_math_eltwise_unary_sfpu_ln_rsqrt_init() {
    llk_math_eltwise_unary_sfpu_init<SfpuType::ln_rsqrt, APPROXIMATE>();
}
