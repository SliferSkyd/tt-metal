...
from models.demos.sentence_bert.ttnn.common import get_mesh_mappers
from models.utility_functions import run_for_wormhole_b0


def run_e2e_performant_sentencebert(device, batch_size, sequence_length, act_dtype, weight_dtype):
    input_mappers, weight_mapper, output_composers = get_mesh_mappers(device)
    performant_runner = SentenceBERTPerformantRunner(
        ...
        inputs_mesh_mapper=input_mappers,
        weights_mesh_mapper=weight_mapper,
        output_mesh_composer=output_composers,
    )
