...
def evaluation(
    ...
    for ttnn_im, im, im0s in preprocessed_images:
        ...
        if model_name == "YOLOv4":
            ...
            from models.experimental.yolov6l.demo.demo_utils_2 import rescale, non_max_suppression, Results
            ...
            results = []
            if len(det):
                det[:, :4] = rescale(im.shape[2:], det[:, :4], im0s[0].shape).round()
                results.append(Results(im0s[0], path=source_list[index], names=classes, boxes=det))
