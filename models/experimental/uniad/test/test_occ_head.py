...
from models.experimental.uniad.tt.tt_occ_head import TtOccHead
...
def test_occhead(reset_seeds, device):
    ...
    _, pcc = assert_with_pcc(torch_out["seg_gt"], tt_output["seg_gt"], 0.0)
    logger.info(f"pcc: {pcc}")
    _, pcc = assert_with_pcc(torch_out["ins_seg_gt"], tt_output["ins_seg_gt"], 0.0)
    logger.info(f"pcc: {pcc}")
    _, pcc = assert_with_pcc(torch_out["seg_out"], tt_output["seg_out"], 0.0)
    logger.info(f"pcc: {pcc}")
    _, pcc = assert_with_pcc(torch_out["ins_seg_out"], tt_output["ins_seg_out"], 0.0)
    logger.info(f"pcc: {pcc}")
